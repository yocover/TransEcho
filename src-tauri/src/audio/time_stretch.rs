...
///
/// This implementation is optimized for low-latency speech acceleration
/// in the 1.0x ~ 1.25x range. It preserves pitch better than naive
/// resampling by selecting overlap points with the highest waveform
/// similarity before crossfading.
...
impl TimeStretchProcessor {
    ...
    fn find_best_overlap(
        ...
        for start in min_start..=max_start {
            ...
            let distance =
                (start as isize - search_center as isize).unsigned_abs() as f32;
    ...
    fn overlap_add(&mut self, frame: &[f32]) {
        ...
        for i in 0..self.overlap_size {
            ...
            self.output_tail[base + i] =
                self.output_tail[base + i] * fade_out + frame[i] * fade_in;
        }

        self.output_tail.extend_from_slice(&frame[self.overlap_size..]);
    }
...
mod tests {
    ...
    fn shortens_audio_when_speeding_up() {
        let mut processor = TimeStretchProcessor::new(1.2);
